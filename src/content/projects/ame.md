...
AME-2 编码器首先利用卷积神经网络（CNN）提取局部地图特征，并通过多层感知机（MLP）计算每个点的位置嵌入；随后，这些信息经由另一个 MLP 融合，生成逐点的局部特征。
接下来，利用一个 MLP 对逐点特征进行处理，并辅以最大池化操作，从而提取出能够捕捉整体地形上下文的全局特征。我们将这些全局特征与本体感知嵌入结合，通过 MLP 生成查询向量（query vector）；该向量随后输入多头注意力（MHA）模块，其中逐点局部特征充当键（key）和值（value）。这一过程生成了加权的局部特征嵌入，使其能够根据当前的本体感知状态及全局上下文，聚焦于重要的地形区域。最后，将全局特征与加权局部特征进行拼接，构成地图嵌入，并将其输入到策略网络的动作解码器中。

该设计基于先前研究中的注意力机制地图编码器（AME-1）构建。与 AME-1 编码器不同，我们的 AME-2 编码器额外计算了全局特征，并利用这些特征对局部特征进行加权。这种机制使得机器人在复杂地形上能够实现更具通用性的运动，因为运动模式应当随地形的变化而相应调整。
